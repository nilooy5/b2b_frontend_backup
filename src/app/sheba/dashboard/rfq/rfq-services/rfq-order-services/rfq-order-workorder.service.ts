...
export class RfqOrderWorkorderService {
    ...
    getAdvancePaymentRequestsList(rfqId, bidId) {
        return this.http.get('v2/businesses/' + this.storage.user.business_id + '/procurements/' + rfqId + '/bids/' + bidId + '/work-order').pipe(catchError(err => {
            return of([]);
        })).pipe(map(res => {
