...
export class RfqQuotationService {
  ...
  constructor(private http: ShebaHttpService,
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<any> {
        return this.getQuotationDetails(route.params.id);
    }

    getQuotationDetails(bidId: number) {
        const url = `v2/businesses/${this.storage.user.business_id}/bids/${bidId}`;
