...
@Component({
    selector: 'app-fleet-request-add',
    templateUrl: './fleet-request-add.component.html',
    styleUrls: ['./fleet-request-add.component.scss']
})
export class FleetRequestAddComponent extends FormEssentials implements OnInit {
    vehicles: Vehicle[] = [];
    members: Member[] = [];
    tripTypes = TripTypes;
    ...
    submit() {
        if (this.form.valid) {
            ...
            data.member_id = this.storage.user.member_id;
            this.submitting = true;
            this.service.vehicleRequest(data).toPromise().then(res => {
                this.submitting = false;
                if (res.code === 200) {
                    this.service.request_id = res.id;
                    this.router.navigate(['/', 'dashboard', 'fleet-management', 'requests', 'add', 'success']).catch(err => {
                        console.log(err);
                    });
                } else {
